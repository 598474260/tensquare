...
public class AdminService {
    ...
    public Admin login(Admin admin) {
        //1.根据登录名查询管理员
        Admin logAdmin = adminDao.findByLoginname(admin.getLoginname());
        //2.如果为null返回null
        if (null == logAdmin) {
            return null;
        }
        //3.不为null判断密码是否正确,不正确返回null
        if (bCryptPasswordEncoder.matches(admin.getPassword(), logAdmin.getPassword())) {
            return logAdmin;
        } else {
            return null;
        }
